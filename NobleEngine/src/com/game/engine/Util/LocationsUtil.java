...
public class LocationsUtil
{
	...
	public static List<Vector> getCircleInVectors(Vector vector, int radius, int height, boolean hollow, boolean sphere)
	{
	...
	public static void pullTo(Entity e, Location loc, boolean inline)
	{
		...
		double y = (1.0 + 0.03 * d) * (loc.getY() - l.getY()) / d + (inline ? 0 : -0.5 * g * d);
		double z = (1.0 + 0.07 * d) * (loc.getZ() - l.getZ()) / d;
		Vector v = e.getVelocity();
		v.setX(x);
		v.setY(y);
		v.setZ(z);
		v.multiply(inline ? 1.5 : 1.0);
		e.setVelocity(v);
	}

	public static List<Location> getAllBlocksInsideCuboid(Location corner1, Location corner2)
	{
	...
	public static boolean isInsideCuboid(Location loc, Location corner1, Location corner2)
	{
		...
		{
			double[] a =
			{ Math.floor(corner1.getX()), Math.floor(corner1.getY()), Math.floor(corner1.getZ()) };
			double[] b =
			{ Math.floor(corner2.getX()), Math.floor(corner2.getY()), Math.floor(corner2.getZ()) };
			if (a[0] > b[0])
			{
				double temp = b[0];
				b[0] = a[0];
	...
	public static List<Location> getCuboid(Location corner1, Location corner2, boolean getOnlyAboveGround)
	{
		...
		for (int x = Math.min(corner1.getBlockX(), corner2.getBlockX()); x <= Math.max(corner1.getBlockX(), corner2.getBlockX()); x++)
			for (int z = Math.min(corner1.getBlockZ(), corner2.getBlockZ()); z <= Math.max(corner1.getBlockZ(), corner2.getBlockZ()); z++)
			{
				for (int y = Math.min(corner1.getBlockY(), corner2.getBlockY()); y <= Math.max(corner1.getBlockY(), corner2.getBlockY()); y++)
				{
					...
					if ((b.getType().equals(Material.AIR)) && ((!getOnlyAboveGround) || (b.getRelative(BlockFace.DOWN).getType() != Material.AIR))) blocks.add(b.getLocation());
				}
			}
		return blocks;
	}

	public static List<Location> getLineBetweenPositions(Location position1, Location position2)
	{
	...
	public static Chunk getChunkOfBlock(Block b)
	{
		if (b.getWorld() == null) return null;
		return b.getWorld().getChunkAt((b.getX() >> 4), (b.getZ() >> 4));
	}

	public static boolean isBlockInsideChunk(Block b, Chunk c)
	{
		return (((b.getX() >> 4) == c.getX()) && ((b.getZ() >> 4) == c.getZ()));
	}

	public static double getDistanceBetweenLocations(Location l1, Location l2)
	{
		return Math.sqrt((Math.pow(Math.abs(l1.getX() - l2.getX()), 2)) + (Math.pow(Math.abs(l1.getY() - l2.getY()), 2)) + (Math.pow(Math.abs(l1.getZ() - l2.getZ()), 2)));
	}
}
